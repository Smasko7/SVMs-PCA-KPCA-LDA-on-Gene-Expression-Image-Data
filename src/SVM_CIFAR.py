...
def run_svm_experiment(X_train, y_train, X_test, y_test,
                       ...
                       pca_threshold, pca_enable, grayscale, k_folds, class_labels):
    ...
    if cores != -1:
        run_id = f"CIFAR-10_{kernel}_{scaler_name}_{cores}_Cores"
    else:
        run_id = f"CIFAR-10_{kernel}_{scaler_name}"
...
def main():
    ...
    cfg = Utils.load_config('CIFAR-10')
    PCA_enable = True
    Multiple_cores_enable = False
    GrayScale = False

    class_labels = Utils.get_class_labels('CIFAR-10')
